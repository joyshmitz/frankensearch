...
//! `limit_all` (k = N) that is N short-`String` allocations. The `PERF_LEDGER`
//! estimated eliding them (the invasive `VectorHit<'a>` borrow refactor) at "~3-5%
//! of `limit_all`" — but that was never measured. This isolates the actual cost: building
