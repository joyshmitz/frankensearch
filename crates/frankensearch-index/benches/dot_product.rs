...
fn bench_dot(c: &mut Criterion) {
    ...
    for &dim in &[256_usize, 384] {
        ...
        // Runtime-AVX2 dispatch (`fourbit_prepared_new` above) vs the portable
        // `wide`-SIMD kernel it falls back to. Bit-identical; AVX2 win on AVX2 hosts.
        group.bench_function(BenchmarkId::new("fourbit_prepared_generic", n), |b| {
            b.iter(|| {
                let mut acc = 0_i64;
                for v in &corpus.stored_4bit {
                    acc += i64::from(dot_4bit_prepared_generic(
                        black_box(v),
                        black_box(&q4_prepared),
                    ));
                }
                black_box(acc)
            });
        ...
        // Runtime-AVX2 dispatch (`f32_bytes_new`) vs the portable `wide` fallback.
        // Bit-identical; 256-bit-width win on AVX2 hosts (no decode win for f32).
        group.bench_function(BenchmarkId::new("f32_bytes_generic", n), |b| {
            b.iter(|| {
                let mut acc = 0.0_f32;
                for bytes in &corpus.stored_f32_bytes {
                    acc += dot_product_f32_bytes_f32_generic(black_box(bytes), black_box(q));
                }
                black_box(acc)
            });
        });
        ...
        // Runtime-AVX2 dispatch (`f32_slice_new`) vs the portable `wide` fallback.
        group.bench_function(BenchmarkId::new("f32_slice_generic", n), |b| {
            b.iter(|| {
                let mut acc = 0.0_f32;
                for v in &corpus.stored_f32 {
                    acc += dot_product_f32_f32_generic(black_box(v), black_box(q));
                }
                black_box(acc)
            });
        });
