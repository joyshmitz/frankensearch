//! FSVI 4-bit (16-level) two-pass vs exact f16 + int8 two-pass — standalone large-N
//! vector search on clustered data.
//!
//! `VectorIndex::search_top_k_4bit_two_pass` packs each vector to signed 4-bit
//! nibbles (`dim/2` bytes — half the int8 slab), runs a parallel `dot_packed_4bit`
//! pass-1, then an exact f16 rescore of the top `k·mult`. 16 levels are lossless at
//! a feasible mult on clustered data (probe: recall@10 = 1.0 at mult=5, N=10k), and
//! the pass-1 reads half the int8 bandwidth — so this should beat the int8 two-pass.
//! This bench confirms recall at N=100k and measures the latency vs int8 + exact.
//!
//! Run with:
//! ```bash
//! CARGO_TARGET_DIR=/data/projects/.rch-targets/frankensearch-cc \
//!   rch exec -- cargo bench -p frankensearch-index --bench fsvi_4bit_two_pass
//! ```
...
fn bench_fsvi_4bit_two_pass(c: &mut Criterion) {
    let dir = std::env::temp_dir().join(format!("fsvi_4bit_two_pass_{}", std::process::id()));
    ...
    // ── Recall@K sweep over candidate_multiplier (confirm lossless at 100k). ──
    for mult in [2usize, 5, 10, 20] {
        ...
        eprintln!(
            "[fsvi_4bit_two_pass] N={N} dim={DIM} k={K} mult={mult} recall@{K}={:.4}",
            total / QUERIES as f64
        );
    }

    // ── Latency: exact flat vs int8 (mult=5) vs 4-bit (mult=5, 10). ──
    let mut qi = 0usize;
    let mut g = c.benchmark_group("fsvi_4bit_two_pass");
    ...
    g.bench_function("int8_mult5", |b| {
        b.iter(|| {
            let q = &queries[qi % QUERIES];
            qi += 1;
            black_box(
                index
                    .search_top_k_int8_two_pass(black_box(q), K, 5)
                    .expect("int8"),
            )
        });
    });
...
criterion_group!(benches, bench_fsvi_4bit_two_pass);
