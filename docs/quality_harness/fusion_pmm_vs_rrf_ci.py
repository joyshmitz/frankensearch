#!/usr/bin/env python
"""Statistical rigor for the FUSION-default question: is pool-min-max score fusion (a9e53b4,
the +0.017 scifact finding) ROBUSTLY better than RRF rank fusion across corpora, or partly
single-run noise? Bootstrap 95% CI on the per-query nDCG@10 delta (pool-min-max - RRF),
stem+stop lexical + model2vec dense, top-100 pool, per corpus + POOLED. 2000 resamples,
seed=12345. Fresh vs existing: cross_corpus_fusion.py is single-run point estimates;
bootstrap_ci.py CIs the down-weight delta, not the fusion-method delta.
"""
...
def per_query_deltas(ds, model, POOL=100):
    ...
    for q in q_ids:
        ...
        deltas.append(n_mm - n_rrf); mm_abs.append(n_mm); rrf_abs.append(n_rrf)
    return np.array(deltas), np.array(mm_abs), np.array(rrf_abs)
...
def main():
    ...
    pooled = []
    print("  corpus     q |   mm     rrf   | mean_delta(mm-rrf)   95% CI            CI>0")
    for ds in datasets:
        d, mm, rrf = per_query_deltas(ds, model); pooled.append(d)
        m, lo, hi = boot_ci(d, rng)
        print(f"{ds:>10} {len(d):>5} | {mm.mean():.4f} {rrf.mean():.4f} |   {m:+.4f}          [{lo:+.4f}, {hi:+.4f}]   {lo > 0}")
