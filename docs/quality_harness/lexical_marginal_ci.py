#!/usr/bin/env python
"""Mirror of dense_marginal_ci.py: the LEXICAL tier's marginal value over dense-alone, so the
tier asymmetry can be read with rigor. dense_marginal_ci showed dense over lexical is a WASH on
arguana; this shows whether lexical over dense is decisive EVERYWHERE (the campaign claim that
lexical is the indispensable tier). Bootstrap 95% CI on the per-query nDCG@10 delta
(pool-min-max hybrid - dense-only), stem+stop lexical + model2vec dense, top-100 pool, per
corpus + POOLED. 2000 resamples, seed=12345.
"""
...
def per_query_deltas(ds, model, POOL=100):
    ...
    for q in q_ids:
        ...
        den = {idx2id[i]: cos[i] for i in dtop}                         # dense-only
        n_h = ndcg(sorted(hyb, key=lambda d:-hyb[d]), rel)
        n_d = ndcg(sorted(den, key=lambda d:-den[d]), rel)
        deltas.append(n_h - n_d); hy_abs.append(n_h); den_abs.append(n_d)
    return np.array(deltas), np.array(hy_abs), np.array(den_abs)
...
def main():
    ...
    pooled = []
    print("  corpus     q |  hybrid  dense | lexical_marginal(h-dense)   95% CI            CI>0")
    for ds in datasets:
        d, hy, den = per_query_deltas(ds, model); pooled.append(d)
        m, lo, hi = boot_ci(d, rng)
        print(f"{ds:>10} {len(d):>5} | {hy.mean():.4f} {den.mean():.4f} |   {m:+.4f}                [{lo:+.4f}, {hi:+.4f}]   {lo > 0}")
