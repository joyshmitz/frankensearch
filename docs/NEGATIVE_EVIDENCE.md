...
|------|-------|----------|----------|--------|
| 2026-06-27 | AmberField (`frankensearch-cod-a`) | `frankensearch/search_bench bold_verify_tantivy_class` clean hot-span retry | No checked bench worktree had commits ahead of `main` (`git rev-list --count main..HEAD` = 0). In isolated worktree `amberfield-lexical-trace-20260627`, retried the prior two-attribute hot-span lever (`execute_query_with_offset` + `TantivyIndex::search_doc_ids`), but the only available comparator artifact was stale (`/data/projects/.rch-targets/frankensearch-cod-a/criterion/bold_verify/summary.jsonl`, `git_sha=3b19d7f2...`) and the fresh BOLD attempt either flooded close-event tracing when the remote env was misplaced or fell open locally with `active_project_exclusion` before producing a new summary. Accepted Lucene/Tantivy/Meili-class ratio: none; source hunk reverted. | Surface only; no code landed. Agent Mail DB was corrupt, so no reservation/message could be recorded. |
