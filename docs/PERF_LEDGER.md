...
| 2026-07-01 | frankensearch-fusion | **async `TwoTierSearcher::search_collect_with_text` skips the wasted Initial-phase clone (`emit_intermediate` flag) — same lever as sync `want_phases` (`5100734`), async 2nd call site** — the async `search()` streams every phase to `on_phase`, cloning the full `Vec<ScoredResult>` into each; `search_collect_with_text` (used by `search_collect` **and federated** per-shard `collect_shard_results`) keeps only the *latest* phase, so when a quality tier runs the Initial-phase `display_hits.clone()` is built → stored → overwritten by Refined → dropped: pure waste. Split `search()` into `search_inner(emit_intermediate)`: pub streaming `search()` passes `true`; `search_collect_with_text` passes `false`, guarding the Initial emit with `emit_intermediate \|\| !quality_will_run` (Initial still fires when it is the terminal phase — no quality tier) and capturing `RefinementFailed.initial_results` so the failure path still delivers. **Bit-identical**: conformance **820 passed, 0 failed** across quality-success/timeout/error/streaming/collect paths. Eliminates **one** `Vec<ScoredResult>` phase clone; that clone's cost is measured directly by the committed `collect_limit_all` bench on the sync sibling (`iter`=2 clones → `collect`=0 = 27.661 → 21.372 ms ⇒ **~3.1 ms per clone at N=10k**, ~14 % of the collect path). Direct async A/B (`async_collect_phase`) blocked this session (embed `hash` feature off in fusion deps + pathologically slow rch workers); the eliminated work is the same clone `collect_limit_all` measures, so this row cites that — NOT an independent additional measurement. | (same clone as `collect_limit_all/iter→collect`, N=10k) | 3.1 ms/clone (measured) | 0 | **~1.1× async collect (1.29× sync, 2-clone)** | KEEP (BlackThrush) |
